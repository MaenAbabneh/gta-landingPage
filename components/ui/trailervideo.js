...
function TrailerOverlay() {
  ...
  useEffect(() => {
    if (isOpen) {
      setVideoID(selectedTrailer?.video_ID || initialVideoID);
    }
  }, [isOpen, selectedTrailer, initialVideoID]);
  ...
  return (
    ...
    >
      <Dialog.Portal container={document.body} forceMount>
        <Dialog.Content
          id="trailers"
          ref={ContainerRef}
          className="fixed inset-0 z-[98] p-2 sm:p-4 md:p-0 overflow-hidden"
          onEscapeKeyDown={(e) => {
            e.preventDefault();
            onClose();
          }}
        ...
        >
          <Dialog.Title className="sr-only">Trailers</Dialog.Title>
          <Dialog.Description id="trailers-desc" className="sr-only">
            Watch the latest trailers for our game.
          </Dialog.Description>
          ...
          <Dialog.Close asChild>
            <button
              className="absolute top-8 right-8 md:top-4 md:right-4 lg:right-6 lg:top-6 z-[99] bg-gta-gray text-white rounded-full w-15 h-15 sm:w-12 sm:h-12 flex flex-col items-center justify-center hover:bg-gta-gray-dark transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/60 cursor-pointer"
              onClick={onClose}
              ref={buttonRef}

  ...
  return (
    <div className="w-full h-full space-y-6 pt-5">
      ...
      {gtaData.Trailers.map((trailer, index) => (
        <div
          key={index}
          className="flex flex-col md:flex-row justify-satrt items-start md:items-center max-w-60  md:max-w-150  min-h-50 md:min-h-40  bg-white/5 hover:bg-white/10 cursor-pointer transition-all duration-300"
          onMouseEnter={() => onHover({ image: trailer.thumbnail })}
          onMouseLeave={onLeave}
          onClick={() => handleTrailerClick(trailer)}
        >
          <div className="relative w-full  min-h-40 md:max-w-60 md:max-h-40 lg:max-w-60 lg:min-h-40 xl:max-w-80 xl:min-h-45">
            <Image
